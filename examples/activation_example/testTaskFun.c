...
int main(void) {
    ...
    int test = TASK_FUN;
    int modeACT =
        MOD_NOW; /* {MOD_NOW=0, MOD_DEF_OFFSET=1, MOD_DEF_NO_OFFS = 2} */
    int act_flag = NOW;
    ...
    modeACT = select_act(
        v_offset,
        num_tasks); // {MOD_NOW=0, MOD_DEF_OFFSET=1, MOD_DEF_NO_OFFS = 2}
    if (modeACT == -1) {
        allegro_exit();
        return 0;
    }
    ...
    textprintf_ex(
        screen, font, 2, 25 + 15, FGC, BGC,
        " Time reference t0 = %ld (time creation/visualization of this screen)",
        time_t0);
    ...
    for (i = 0; i < num_tasks; i++) {
        ...
        if (modeACT != MOD_DEF_OFFSET) {
            ...
            if (act_flag == NOW) {
                pthread_mutex_lock(&mxa);
                textprintf_ex(screen, font, X_LINE_VERT0 + 5,
                              Y_TIMES + i * ALT_RIGA, FGC, BGC,
                              "%ld            ", time_tmp);
                pthread_mutex_unlock(&mxa);
            }
            ...
            time_tmp = ptask_gettime(MILLI);
            pthread_mutex_lock(&mxa);
            textprintf_ex(screen, font, (X_LINE_VERT0 + X_LINE_VERT1) / 2,
                          Y_TIMES + i * ALT_RIGA, FGC, BGC, "-");
            pthread_mutex_unlock(&mxa);
            id = ptask_create_prio(periodicLine_testSystemTaskOFFSET, PER,
                                   priority[i], act_flag);
            ...
            if (ret != -1) {
                ...
                if (i == 0) {
                    pthread_mutex_lock(&mxa);
                    textout_ex(screen, font, str_offset, XMIN,
                ...
                textprintf_ex(screen, font,
                              XMIN +
                                  (strlen(str_offset) + i * (NUM_CIF_OFFSET)) *
                                      PIXEL_CHAR,
    ...
    do {
        ...
        if ((k >= KEY_1) && (k <= KEY_0 + num_tasks) &&
            (modeACT != MOD_DEF_OFFSET)) {
