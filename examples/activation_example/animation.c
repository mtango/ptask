...
void periodicLine_testSystemTask() {
    ...
    textprintf_ex(screen, font, X_LINE_VERT1 + 5, Y_TIMES + i * ALT_RIGA, FGC,
                  BGC, "%ld", time_activation);
    ...
    while (x < XMAX) {
        ...
        if ((x >= XMAX / 2) && !bloccato) {
            ...
            pthread_mutex_lock(&mxa);
            textprintf_ex(screen, font, X_LINE_VERT1 + 5,
                          Y_TIMES + i * ALT_RIGA, FGC, BGC, "%ld",
                          time_activation);
            pthread_mutex_unlock(&mxa);
        }
    }
...
void periodicLine_testSystemTaskOFFSET() {
    ...
    snprintf(id, 3, "T%d", i + 1);
    pthread_mutex_lock(&mxa);
    textprintf_ex(screen, font, X_LINE_VERT2 + 5, Y_TIMES + i * ALT_RIGA, FGC,
                  BGC, "%ld", offset_misurato);
    textprintf_ex(screen, font, X_LINE_VERT1 + 5, Y_TIMES + i * ALT_RIGA, FGC,
                  BGC, "%ld", time_start);
    textout_ex(screen, font, id, 5, y, FGC, 0);
    pthread_mutex_unlock(&mxa);
