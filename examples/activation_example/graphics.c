...
const char *string_act_mode[] = {" ACTIV. MODE = NOW",
                                 " ACTIV. MODE = DEFERRED (with time offset)",
                                 " ACTIV. MODE = DEFERRED (no time offset)"};
                      ...
                      int tipo_test, bool verbose)
{
    ...
    if (tipo_test == SCHED)
        ...
        textprintf_ex(screen, font, 2 + 50, 25, col_act_flag, BGC, "%s",
                      string_act_mode[act_mode]);
    }
    ...
    textout_ex(screen, font,
               "KEY [1-9] to activate tasks (mod != MOD_DEF_OFFSET)", 5,
               YWIN - 20, 10, 0);
...
void draw_Timetask_info(int modeACT)
{
    ...
    if (modeACT == MOD_DEF_OFFSET) {
        textout_ex(screen, font, "Time offset(ms) inseriti = ", XMIN,
                   BASE1 + 2 * ALT_RIGA, FGC, 0);
    }

    textout_ex(screen, font, "TIME ACTIVATION(ms)", X_LINE_VERT0 + 5,
               Y_ORRIZ_LINE - ALT_RIGA, FGC, 0);
    textout_ex(screen, font, "TIME START(ms)", X_LINE_VERT1 + 5,
               Y_ORRIZ_LINE - ALT_RIGA, FGC, 0);
    textout_ex(screen, font, "TIME OFFSET measured(ms)", X_LINE_VERT2 + 5,
               Y_ORRIZ_LINE - ALT_RIGA, FGC, 0);

    for (i = 0; i < NUM_T_TEST; i++) {
        textprintf_ex(screen, font, XMIN, Y_TIMES + i * ALT_RIGA, FGC, BGC,
                      "T%d:", i + 1);
    }
}
...
{
    static char *strings[] = {"NOW", "DEFERRED with OFFSET",
                              "DEFERRED without OFFSET"};
    if (index < 0) {
        *list_size = 3;
        return NULL;
    }
...
DIALOG taskFunction_dialog[(9 + NUM_T_TEST * 2)] = {
    ...
    {d_ctext_proc, XWIN / 2 - 150, 30, 300, 50, 14, BGC, 0, 0, 0, 0,
     "----- TASK FUNCTION TEST -----", NULL, NULL},

    {d_box_proc, 20, YWIN / 2 - 80, 600, 50, FGC, BGC, 0, 0, 0, 0, NULL, NULL,
     NULL},
    {d_text_proc, XMIN, YWIN / 2 - 60, 80, 20, FGC, BGC, 0, 0, 0, 0,
     "ACTIVATION FLAG setting", NULL, NULL},
    {d_list_proc, XWIN / 2 + 50, YWIN / 2 - 70, 200, 30, FGC, 0, 0, 0, 0, 0,
     listboxAct_getter, NULL, NULL},
...
void activate_offset_box(int var)
{
    ...
    for (i = 7; i <= (NUM_T_TEST * 2 + 7); i++) {
        ...
        if (var == ON)
            taskFunction_dialog[i].fg = FGC;
        else if (var == OFF)
            taskFunction_dialog[i].fg = 0;
    }
}

/* It draws dinamically  the offset's field */
void update_dialogAct(char textOffset[NUM_T_TEST][W_TEXT_OFFSET + 1],
                      char editOffset[NUM_T_TEST][NUM_CIF_OFFSET + 1])
{
    ...
    // general structure: proc, x, y, w, h, fg, bg, key, flags, d1, d2, dp, dp2,
    // NULL
    DIALOG d_temp_text = {d_text_proc,
                          XMIN,
                          d7.y + (d7.h) / 2,
                          W_TEXT_OFFSET *
                              PIXEL_CHAR, /*weight text in pixel 72*/
                          20,
                          FGC,
                          BGC,
                          0,
    ...
    DIALOG d_temp_edit = {d_edit_proc,
                          XMIN + d_temp_text.w,
                          d7.y + (d7.h) / 2,
                          (NUM_CIF_OFFSET + 1) *
                              PIXEL_CHAR, // W_EDIT_PRIO * PIXEL_CHAR,
                          20,
                          FGC,
                          BGC,
                          0,
    ...
    for (i = 0; i < NUM_T_TEST; i++) {
        ...
        taskFunction_dialog[j].x =
            XMIN + i * (d_temp_text.w + d_temp_edit.w + offset);
...
int select_act(ptime *v_offset, int numTask)
{
    ...
    char *endptr;
    char text_offset[NUM_T_TEST]
                    [W_TEXT_OFFSET +
                     1]; // all strings OFFSET "Px" (ex.text_offset[0]="Offs0:")
    char edit_offset[NUM_T_TEST]
                    [NUM_CIF_OFFSET + 1]; // all strings of edit fields
    ...
    while (update_dialog(player)) {
        ...
        if (mode == MOD_DEF_OFFSET) {
            ...
            /* ex.: if tasks are 5, the index of editable fields to checking are
             * 12 14 16 18 20:
            ...
            for (i = 9; i <= (NUM_T_TEST * 2 + 7) && i != id; i = i + 2) {
                ...
                if (error_edit(taskFunction_dialog[i].dp, "offset", endptr, val,
                               OFFSET_MAX)) {
                    snprintf(taskFunction_dialog[i].dp, NUM_CIF_OFFSET + 1,
                             "%d", OFFSET_MAX);
                    taskFunction_dialog[i].flags = D_DIRTY;
                }
            }
